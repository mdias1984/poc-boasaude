...
(function($) {
    ...
    function getImageThumbnailHtml(src) {
    	...
    	$('.thumbnail').css('max-width', options.maxWidth);
    	$('.thumbnail').css('max-height', options.maxHeight);
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'O arquivo é muito grande (max ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            ...
            callback(false, 'Arquivo não permitido.');
        }
    }
    ...
    function submitImageFile($fileTab) {
        ...
        var $removeFileButton 	= $fileTab.find('.btn:eq(1)');
        var $fileInput 			= $fileTab.find('input');
        var $fileHidden 		= $('input[type="hidden"][name="foto"]');
        var $fileType 			= $('input[type="hidden"][name="fotoTipo"]');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                	...
                	getImageThumbnailHtml(e.target.result);
